   ...
   <!--     <a href="<?php echo site_url('orders/tracking') ?>" class="border-bottom">Lacak Pesanan</a>-->
        ...
        ?>
           ...
           <!-- <a href="<?php echo site_url('coupons') ?>" class="border-bottom">Voucher</a> -->
            <a href="<?php echo site_url('admin/login/logout') ?>" class="border-bottom">Keluar</a>
        <?php
        }
        ?>
    </div>
...
<div class="nav-scroller py-1" style="margin-top:60px;">
    <div class="container">
        <div class="row d-flex text-center kanankiri">
            <div class="col-lg-12">
                <nav class="nav ">
                    <a class=" text-decoration-none pt-2 pe-4 pb-2 text-success " href="<?php echo site_url('produks') ?>">All Products</a>
                    <?php
                    $this->db->select('katProduk');
                    $this->db->from('produk');
                    $this->db->group_by('katProduk');
                    $querykat = $this->db->get();
                    $resultkat = $querykat->result_array();

                    foreach ($resultkat as $row) {
                        $katproduk = str_replace(" ", "-", $row['katProduk']);
                    ?>
                        <a class=" text-decoration-none pt-2 pe-4 pb-2 text-success" href="<?php echo site_url('produks/kategori/' . $katproduk) ?>">
                            <?php echo "$row[katProduk]"; ?>
                        </a>
                    <?php
                    }
                    ?>
...
<a href='https://api.whatsapp.com/send?phone=6281261700028&text=Halo%20Kak,%20mau%20info%20produknya%20dong?' class="floatwa">
